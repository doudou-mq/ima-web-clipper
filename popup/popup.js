...
// 操作日志由共享模块提供（lib/logger.js）：log / loadLogs / flushLogs / clearLogs / srcMeta / SRC_META
// 本文件通过 setLogOnChange 注册回调，有新日志时刷新日志面板
...
async function initApp() {
  initDOM();
  bindEvents();
  setLogOnChange(function() { if (els.logPanel && !els.logPanel.hidden) renderLogs(); });
  await loadLogs();
...
function bindEvents() {
  ...
  els.logClear.addEventListener('click', function() { clearLogs(); log('INFO', 'log', '日志已清空'); });
